...
fun Route.customerRouting() {
    ...
    route("/customer") {
        ...
        post {
            val customer = call.receive<Customer.Customer>()
            customerStorage.add(customer)
            call.respondText("Customer stored correctly", status = HttpStatusCode.Accepted)
        }
        delete("{id}") {
            val ida = call.parameters["id"] ?: return@delete call.respond(HttpStatusCode.BadRequest)
            if (customerStorage.removeIf { it.id == ida }) {
